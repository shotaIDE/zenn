...
- タグプッシュ時のユーザーテスト用のアプリのデプロイ
- タグプッシュ時の Cloud Functions の自動デプロイ
- タグプッシュ時の一般公開用のアプリのデプロイと審査提出＆公開

また、Fastlane を利用して上記で実行される内容を 1 つのコマンドで行えるようにしています。
これにより、ローカル用と CI/CD 用で同じスクリプトを二重に書いておく必要がなくなります。
