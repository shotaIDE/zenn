...
基本的に、特定のコミットだけリリースから除外するのは、リリース戦略が複雑になるため避けるべきです。

:::message
特定のコミットだけリリースから除外することを避けるために、以下のような手段が取れます。

- タスクの選択と集中をすることで、マージからテスト完了までのリードタイムを短くする
- 高頻度のリグレッションテストや包括的な自動テストを用意する
...
定期リリースの 2 回目以降のビルドとほぼ同等の内容です。

2 回目以降のビルドで「リリースのためのテストに対するバグ修正」となっていた箇所が「hotfix として含めたい修正」となっているだけです。
