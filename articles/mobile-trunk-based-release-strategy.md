...
**安全な開発を脅かしたり開発スピードを下げたりする以下の 2 要素を徹底的に禁止**することで、安全で素早い開発を継続していきます。

- 大規模または長期にわたるコード修正を特定のエンジニアだけで閉じて行う
- 修正したコードを一時的にマージしない期間を設ける

### 大規模または長期にわたるコード修正を特定のエンジニアだけで閉じて行うことの禁止

大規模または長期にわたるコード修正は**認知の負荷やマージの難易度が上がり、それに伴うリスクが高まる**ため、禁止とします。
...
### 修正したコードを一時的にマージしない期間を設けることの禁止

開発者の手元でコード修正が滞留し、コードレビューやデプロイ、テストなど**各種の開発プロセスが停滞してしまう**ため、禁止とします。
