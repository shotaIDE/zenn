...
リリース作業時にのみ、トランクブランチの最新コミットからリリースブランチを分岐させます。
リリースブランチを作成しているのは、以下のような理由からです。

- リリースブランチはコードオーナーを設定し、特定のメンバーしかリリース作業ができないようにするため
- リリース時にはマージのログなどをコミット履歴に残しておき、それを次回のリリースで比較したいため

リリースの際には、トランクブランチにはマージされない前提で、リリースブランチを作成します。

リリースブランチ上でリリース前の総合テストを行います。

総合テストでバグが見つかった場合には、トランクブランチに修正を行い、リリースブランチにチェリーピックします。

この辺りは、奇妙に思えるかもしれませんが、以下のような考えに基づいています。
...
バグが見つかりすぎると、リリースブランチがトランクブランチと乖離しすぎるため、トランクブランチでの修正を行うのは難しくなってくるためです。
...
## (そもそもこの取り組み自体がナンセンスだと考えているが、)リリースに取り込みたい内容を取捨選択するのが難しくなる
