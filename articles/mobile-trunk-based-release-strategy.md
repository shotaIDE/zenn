...
1 年ほど運用してみて、どのような戦略がうまくいっているかをまとめました。

# 達成したいこと

- 1 つのソースコードに対して集中することで、開発プロセスを最適化する
- リリースにおけるリスクやコストを最小化し、リリースの頻度が高められるようにする
...
- 副次的な効果として、フィーチャーフラグをリモートで管理することにより、切り戻しの範囲を柔軟に設定でき、所要時間を最小化できる
...
- 品質を担保した後にマージというフェーズが取らずに、トランクブランチのいつでもある程度致命的なバグが存在しない状態の水準にすることが求められる
