...
安全な開発を脅かしたり開発スピードを下げたりする以下の 2 点を禁止することで、安全で素早い開発を継続していきます。

- 大規模または長期にわたる開発をエンジニアの特定のエンジニアだけで閉じて行う
- 開発したコードを一時的にマージしない期間を設ける

### 大規模または長期にわたる開発をエンジニアの特定のエンジニアだけで閉じて行うことを禁止する

大規模または長期にわたる修正には以下のつらさがあり、それを解消するためです。
...
これは、以下のような**リリースブランチを作り最後にトランクブランチへマージするという営みも行わな**いことを意味します。
...
### 開発したコードを一時的にマージしない期間を設けることを禁止する

本項目は、リリースのためのテストなどを行っている際に、**機能開発やリファクタリングなどの作業が中断することを防ぎたい**ためです。

これにより、開発スピードを最大化できます。
...
エンジニアは、**機能開発やバグ修正などのチケット種別や、ターゲットのリリースバージョンに関わらず、全ての修正をトランクブランチにマージ**します。
