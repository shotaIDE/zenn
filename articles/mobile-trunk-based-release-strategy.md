...
開発したコードをマージしてはいけない期間を設けず、かつ、大規模または長期にわたる開発をエンジニアの特定のメンバーだけで閉じて行わないことを目指します。
...
- 開発ブランチからリリースブランチを分岐させ、受け入れ試験で発生したバグ修正をそこに適用し、最後に開発ブランチへマージします
  - このような戦略は、開発ブランチを触っているメンバーとリリースブランチを触っているメンバーが異なるため、コンフリクトが発生しやすいです
