...
緊急度は高くないけど一定数発生しているようなクラッシュについて、定期的にトリアージを行うようにしました。
...
継続的な価値提供をするためには、予期しないトラブルや対応などのリスクを低減する取り組みが必要です。

以下のようなリスクを下げる取り組みを行いました。

### 4-1. アプリのメトリクスに基づく意思決定

Android のプッシュ通知に関して、受信後の仕組みを EOL に伴い、新しい仕組みに変更する必要がありました。
その際に、サーバーからプッシュ通知を送信した後、Android 端末でどの程度正常に遅延なく表示されているかを収集できる仕組みを作りました。
...
依存するライブラリのライセンス情報一覧を自動で生成し、それらが変化していないかを定期的にチェックするツールを作成しました。

これにより、以下のようなミスを防ぐことができました。

> テストコードのために必要なライブラリがプロダクトコードのビルドに含まれるように変化した。
> これはライブラリの指定方法のミスが原因で意図していないものだったため、リリース前に修正できた。
