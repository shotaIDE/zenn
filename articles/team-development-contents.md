...
以下のように関わるステークホルダーは様々でしたので、さまざまな視点で取り組みを行いました。
...
- iOS/Android それぞれ 4-5 人程度のエンジニアが関わる規模です
...
トランクベース開発は、複雑なブランチ管理を極力行わずに継続的に安定した価値提供を目指せる手法です。
...
ユーザーの手元で動作するプログラムを置き換える「デプロイ」と、ユーザーの手元で新機能が発動する「ロールアウト」について、切り離してプロセス設計することは重要です。
通常、デプロイとロールアウトを同時に行う方法が最も簡単でコストを抑えられますが、デプロイとロールアウトを分離させることで運用上のメリットがあります。
...
- 設計時点で、ロールアウトの具体的な方法を決める
  ...
  - フィーチャートグルを使用する場合、ライフサイクルはどうするか
...
### 4-2. 外部要因によるワークフローが動作しなくなる可能性を下げる
