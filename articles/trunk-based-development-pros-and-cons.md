...
Sun\* でモバイルエンジニアをしているころむにーです。

普段は、モバイルアプリを通じてユーザーへ価値提供することを目指すプロジェクトに、技術リードという立場で参画しています。

私が参画しているプロジェクトにおけるトランクベース開発について、考えていることをまとめてみます。
...
また、リリースの際には、基本的にはトランクブランチ上の最新コミットをそのままリリースします。

機能開発途中のコードがデプロイされユーザーの手元に届くことが普通なので、フィーチャートグルなどを用いてロールアウトの仕組みを併用することが一般的です。
...
- リリース時点のコードに致命的なデグレが含まれていて、リリースの延期が必要となった
...
- バグ修正 A に関して、リリース前の動作確認が間に合わないためリリースから外すことになった。この際に、トランクブランチ上ではバグ修正 A のコード修正をリバートした
