...
- 長期間生存するブランチをマージすることの困難さ
- 一部のメンバーだけで修正を長期間管理することによる、知見の偏在化
- マージブロック期間が発生することによる開発スピードの鈍化

トランクベース開発では「数日以上の生存期間を持つブランチを複数許容しない」というルールを設けます。

それに伴い、以下のような開発フローとなります。

- 開発者はトランクブランチに対して、細かくコミットや PR をマージする
- トランクブランチ上で、テストや CI を実行し、品質を担保する
- リリースの際には、トランクブランチ上の最新コミットをベースにリリースする

機能開発途中のコードがデプロイされユーザーの手元に届くことが普通なので、フィーチャートグルなどを用いてロールアウトを制御する仕組みを併用することがほぼ必須です。
...
また、コード修正をどのブランチにマージすればいいかなどのコミュニケーションがなくなり、**ブランチ管理のコストが大幅に削減されました**。
...
> 「機能 A はデータベースへの書き込みが増加するから負荷が心配だね。パーセンテージロールアウトして、もし問題が起きたらこの機能だけロールバックしようか。」

このように、リリースのリスクへの対策をしつつ、ユーザーへの価値提供を最大限維持することができます。
